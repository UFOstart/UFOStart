...
def company_import(context, request):
    ...
    try:
        ...
        request.fwd_raw(location = request.session.get('ANGELLIST_FURL', request.fwd_url("website_index")))
...
def company_import_list(context, request):
    ...
    return {'companies':context['angellist'].getCompaniesData(user_id, token), 'back_url' : request.session.get('ANGELLIST_FURL', request.fwd_url("website_index"))}
...
def company_import_confirm(context, request):
    ...
    request.fwd_raw(location = request.session.pop('ANGELLIST_FURL', request.fwd_url("website_index")))
